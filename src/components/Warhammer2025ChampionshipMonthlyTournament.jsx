...
export default function Warhammer2025ChampionshipMonthlyTournament() {
  ...
  useEffect(() => {
    async function fetchRounds() {
      ...
      parsedMatches.data.forEach(row => {
        if (row.eventName === 'Warhammer 40000: 2025 Championship Monthly Tournament') {
  ...
  const filteredMatches = currentRound
    ? matches.filter(row =>
        ...
        row.eventName === 'Warhammer 40000: 2025 Championship Monthly Tournament')
    : [];

  let placings = [];
  if (location.pathname === '/events/warhammer-2025-championship-monthly-tournament') {
    const eventMatches = matches.filter(row => row.eventName === 'Warhammer 40000: 2025 Championship Monthly Tournament');
    const playerStats = {};
  ...
  const navBar = (
    <div className="flex flex-wrap gap-2 mb-6">
      {[...Array(roundCount)].map((_, i) => {
        const roundNum = i + 1;
        const to = `/events/warhammer-2025-championship-monthly-tournament-round/${roundNum}`;
      ...
      <Link
        to="/events/warhammer-2025-championship-monthly-tournament"
        className={`px-4 py-2 rounded-lg font-semibold transition-colors duration-200 ${location.pathname === '/events/warhammer-2025-championship-monthly-tournament' ? 'bg-indigo-700 text-white' : 'bg-indigo-100 text-indigo-900 dark:bg-indigo-900 dark:text-indigo-100 hover:bg-indigo-200 dark:hover:bg-indigo-800'}`}
      >
        Results
      </Link>
    </div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-indigo-700 dark:text-indigo-200 mb-4">Warhammer 40000: 2025 Championship Monthly Tournament</h1>
      {navBar}
      {location.pathname === '/events/warhammer-2025-championship-monthly-tournament' ? (
        <div>
          {placings.length > 0 && (
            <div className="mt-8">
