...
export default function TheFarSouthCoastRumbleBega40kRTT() {
  ...
  useEffect(() => {
    async function fetchRounds() {
      ...
      parsedMatches.data.forEach(row => {
        if (row.eventName === 'The Far South Coast Rumble: Bega 40k RTT') {
  ...
  const filteredMatches = currentRound
    ? matches.filter(row =>
        ...
        row.eventName === 'The Far South Coast Rumble: Bega 40k RTT')
    : [];

  let placings = [];
  if (location.pathname === '/events/the-far-south-coast-rumble-bega-40k-rtt') {
    const eventMatches = matches.filter(row => row.eventName === 'The Far South Coast Rumble: Bega 40k RTT');
    const playerStats = {};
  ...
  const navBar = (
    <div className="flex flex-wrap gap-2 mb-6">
      {[...Array(roundCount)].map((_, i) => {
        const roundNum = i + 1;
        const to = `/events/the-far-south-coast-rumble-bega-40k-rtt-round/${roundNum}`;
      ...
      <Link
        to="/events/the-far-south-coast-rumble-bega-40k-rtt"
        className={`px-4 py-2 rounded-lg font-semibold transition-colors duration-200 ${location.pathname === '/events/the-far-south-coast-rumble-bega-40k-rtt' ? 'bg-indigo-700 text-white' : 'bg-indigo-100 text-indigo-900 dark:bg-indigo-900 dark:text-indigo-100 hover:bg-indigo-200 dark:hover:bg-indigo-800'}`}
      >
        Results
      </Link>
    </div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-indigo-700 dark:text-indigo-200 mb-4">The Far South Coast Rumble: Bega 40k RTT</h1>
      {navBar}
      {location.pathname === '/events/the-far-south-coast-rumble-bega-40k-rtt' ? (
        <div>
          {placings.length > 0 && (
            <div className="mt-8">
