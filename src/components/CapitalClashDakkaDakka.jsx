...
function getFactionImage(faction) {
  ...
  const candidates = [];
  // Try original, encoded, no apostrophe, underscores, lowercase
  const variants = [
    trimmed,
...
export default function CapitalClashDakkaDakka() {
  ...
  // Determine number of rounds for navigation
  ...
  useEffect(() => {
    async function fetchRounds() {
      ...
      parsedMatches.data.forEach(row => {
        if (row.eventName === 'Capital Clash - Dakka Dakka!') {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const matchesText = await matchesResp.text();
        const parsedMatches = Papa.parse(matchesText, { header: true, skipEmptyLines: true });
        setMatches(parsedMatches.data); // Keep all matches for navigation, filter for display
  ...
  // Filter matches for the current round (if any)
  const filteredMatches = currentRound
    ? matches.filter(row =>
        String(row.gameNumber) === String(currentRound) &&
        row.eventName === 'Capital Clash - Dakka Dakka!')
    : [];

  // Compute placings for Results page
  let placings = [];
  if (location.pathname === '/events/capital-clash-dakka-dakka') {
    // Get all matches for this event
    const eventMatches = matches.filter(row => row.eventName === 'Capital Clash - Dakka Dakka!');
    const playerStats = {};
  ...
  const navBar = (
    <div className="flex flex-wrap gap-2 mb-6">
      {[...Array(roundCount)].map((_, i) => {
        const roundNum = i + 1;
        const to = `/events/capital-clash-dakka-dakka-round/${roundNum}`;
      ...
      <Link
        to="/events/capital-clash-dakka-dakka"
        className={`px-4 py-2 rounded-lg font-semibold transition-colors duration-200 ${location.pathname === '/events/capital-clash-dakka-dakka' ? 'bg-indigo-700 text-white' : 'bg-indigo-100 text-indigo-900 dark:bg-indigo-900 dark:text-indigo-100 hover:bg-indigo-200 dark:hover:bg-indigo-800'}`}
      >
        Results
      </Link>
    </div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-indigo-700 dark:text-indigo-200 mb-4">Capital Clash - Dakka Dakka!</h1>
      {navBar}
      {location.pathname === '/events/capital-clash-dakka-dakka' ? (
        <div>
          {placings.length > 0 && (
            <div className="mt-8">
              ...
              <div className="grid grid-cols-1 gap-4 mb-6">
                {placings.slice(0, 3).map((p, idx) => {
                  ...
                  // Compute win/loss sequence for this player, in round order
                  ...
                  // Use leaderboard-style podium circle for placing number
                  let podiumClass = '';
